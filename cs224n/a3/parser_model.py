...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings.index_select(0, w.view(-1)).view(-1, self.n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        hidden = torch.matmul(emb, self.embed_to_hidden_weight).add(self.embed_to_hidden_bias)
        hidden = self.dropout(hidden)
        hidden = nn.ReLU()(hidden)
        logits = torch.matmul(hidden, self.hidden_to_logits_weight).add(self.hidden_to_logits_bias)
