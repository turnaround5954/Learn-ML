...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        #     'RA': lambda: self.dependencies.append((self.stack[-2], self.stack.pop(-1)))
        # }
    ...
    while unfinished_parses:
        ...
        for parse, transition in zip(minibatch, transitions):
            ...
            if not ((len(parse.buffer) == 0) and (len(parse.stack) == 1)):
                batch_unfinised.append(parse)
        unfinished_parses = batch_unfinised + unfinished_parses[batch_size:]
