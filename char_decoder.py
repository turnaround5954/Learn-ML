...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        embs = self.decoderCharEmb(input)
        hiddens, dec_hidden = self.charDecoder(embs, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        loss = nn.functional.cross_entropy(scores.flatten(start_dim=0, end_dim=1),
                                           char_sequence[1:].flatten(),
                                           ignore_index=self.target_vocab.char_pad,
                                           reduction='sum')
        # loss = loss_fn(scores.flatten(start_dim=0, end_dim=1), char_sequence[1:].flatten())
        return loss
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        char2id = self.target_vocab.char2id
        ...
        rows_remain = {i for i in range(batch_size)}
        ...
        #                       dtype=torch.long,
        #                       device=device)
        x_t = torch.tensor([start_of_word] * batch_size,
                           dtype=torch.long,
                           device=device).view(1, batch_size).contiguous()
        for _ in range(max_length):
            scores, dec_hidden = self.forward(x_t, dec_hidden)
            x_t = scores.argmax(dim=-1)
            # outputs = torch.cat((outputs, x_t), dim=0)
            new_indices = x_t.flatten().tolist()
            for i in range(batch_size):
                if i not in rows_remain:
                    continue
                if new_indices[i] == end_of_word:
                    rows_remain.remove(i)
                    continue
                decodedWords[i] += id2char[new_indices[i]]
