...
def optimal_step_weights():
    ...
    # *** START CODE HERE ***
    #### hidden layer:
    # 1: x1 > 0.5 which means -0.5 + x1 > 0
    # 2: x2 > 0.5 which means -0.5 + x2 > 0
    # 3: x1 + x2 < 4 which means 4 - x1 - x2 > 0
    ...
    #### output layer:
    # h1 + h2 + h3 < 3 which means 3 - h1 - h2 - h3 > 0
...
def optimal_linear_weights():
    ...
    # if linear activation function is used
    # there's no nonlinearity added to the model
    # so it's a linear classifier.
    # thus, it's impossible to classify nonlinear separable data.
