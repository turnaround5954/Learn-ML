...
def initial_state():
    ...
    # *** START CODE HERE ***
    # represent theta as a linear combination of sample 
    state = []
    return state
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    # *** START CODE HERE ***
    theta_x = 0.
    for coef, value in state:
        theta_x += coef * kernel(value, x_i)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    # *** START CODE HERE ***
    if len(state) == 0:
        coef = learning_rate * (y_i - 1.)
        value = x_i
        state.append((coef, value))
        return
