...
def choose_action(state, mdp_data):
    ...
    return np.argmax(value.dot(trans_probs[state]))

    # score1 = trans_probs[state, :, 0].dot(value)
    # score2 = trans_probs[state, :, 1].dot(value)
    # return np.argmax([score1, score2])
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # estimate transition_probs
    sa_counts = trans_counts.sum(axis=1)
    indices = np.where(sa_counts > 0)
    trans_probs[indices[0], :, indices[1]] = trans_counts[indices[0], :, indices[1]] / sa_counts[indices].reshape(-1, 1)
    ...
    # update data
    # mdp_data['transition_probs'] = trans_probs
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    trans_probs = mdp_data['transition_probs']
    reward = mdp_data['reward']
    value_old = mdp_data['value']

    value = reward + gamma * np.max(value_old.dot(trans_probs), axis=1)
