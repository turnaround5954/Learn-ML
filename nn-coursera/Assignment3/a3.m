...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %forward phase implemented by Andy
  ...
  %backward phase implemented by Andy
  %ret.input_to_hid = model.input_to_hid * 0;
  %ret.hid_to_class = model.hid_to_class * 0;
  m = size(data.targets, 2);
  ret.hid_to_class = (1 / m) * (class_prob - data.targets) * hid_output' + ...
      wd_coefficient * model.hid_to_class;
  ret.input_to_hid = (1 / m) * model.hid_to_class' * (class_prob - data.targets) .* ...
      hid_output .* (1 - hid_output) * data.inputs' + wd_coefficient * model.input_to_hid;
end
